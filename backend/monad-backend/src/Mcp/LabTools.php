...
class LabTools
{
    ...
    #[McpTool(
        ...
        description: <<<'TXT'
            ...
            `route_policy` is null / {mode:fixed} (serve the declared step order, the default and
            what every quest did before IP-145) or {mode:pool,routes:[[target_key,...],...]}. A
            pool is PRE-GENERATED by `monad-knowledge lab quest-build --pool N`, never computed
            here: the rule that keeps a walk from crossing a wall reads PostGIS, which this backend
            cannot see, and a second implementation of it would diverge silently. Each enrollment
            draws one route and it is recorded on the enrollment.
    ...
    public function questWrite(
        ...
        if ($route_policy !== null) {
            $quest->setRoutePolicy($route_policy);
        }
